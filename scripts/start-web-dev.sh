#!/usr/bin/env bash
# Default container CMD: backend :3001 + Flutter Web :5174 + reverse proxy :5173.
set -euo pipefail
...
printf '%s\n' \
  '[dev] Backend API in tmux session: backend-dev  (container :3001, host map 8800:3001)' \
  ...
  '[dev] Proxy routes:  /api + /health   -> backend :3001' \
  ...
  '[dev] Interactive flutter: /app/scripts/flutter-run-web.sh  (API -> :3001, open printed URL)' \
